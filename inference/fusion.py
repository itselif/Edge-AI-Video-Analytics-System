...
def fuse_detections_and_tracks(
    ...
    - Eğer detection var ama IoU < iou_thres ise:
        ...
        (test_fusion_low_iou_triggers_drift tam olarak bunu istiyor.)
    ...
    # --------------------------------------------------
    # 0) Hiç detection yoksa → sadece tracker'ları döndür
    # --------------------------------------------------
    if len(dets) == 0:
        for trk in trks:
    ...
    # --------------------------------------------------
    # 1) Detection var → drift kuralı devrede
    # --------------------------------------------------
    Nd = len(dets)
    Nt = len(trks)
    ...
    if Nd > 0 and Nt > 0:
        ...
        while True:
            ...
            # Bu satır/sütun tekrar kullanılmasın
            ious_copy[d_idx, :] = -1.0
            ious_copy[:, t_idx] = -1.0

    # --------------------------------------------------
    # 3) Eşleşmeyen detection'lar → yeni obje (drift)
    #    track_id = None, source="det_only"
    # --------------------------------------------------
    ...
    #    DİKKAT: Detection VARSA, drift kabul ediyoruz
    #    ve bu track'leri fused çıktısına EKLEMIYORUZ.
    #
    #    Yani burada bilerek hiçbir şey yapmıyoruz.
    # --------------------------------------------------
